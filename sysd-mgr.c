...
static GtkWidget *create_service_list_view(GtkListStore **out_store) {
    ...
    GtkTreeViewColumn *col_name = gtk_tree_view_column_new_with_attributes("Name", r_text, "text", 0, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree), col_name);
    gtk_tree_view_column_set_expand(col_name, TRUE);

    GtkTreeViewColumn *col_state = gtk_tree_view_column_new_with_attributes("State", r_text, "text", 1, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree), col_state);
